...
`DonutShinobu` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/DonutShinobu/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/DonutShinobu/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee DonutShinobu` to
...
python3 ./.trellis/scripts/task.py archive 00-join-donutshinobu
```
