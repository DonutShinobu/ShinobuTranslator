...
describe('GigaViewer TTB image composition', () => {
  ...
  it('stacks two or three equal-width source slices without gaps', async () => {
    const close = vi.fn();
    const bitmaps = [
    ...
    const file = await composeGigaViewerTtbFiles(
      [0, 1, 2].map((index) => new File([String(index)], `${index}.png`)),
  ...
  it('crops a translated logical page back at exact member heights', async () => {
    const close = vi.fn();
    const bitmap = { width: 720, height: 2109, close };
