...
#
# 模型 Release 使用 models-v* 前缀，自动添加并标记为 pre-release
# 上传的文件名不含目录前缀（如 detector.onnx）
# Release URL 格式: https://github.com/DonutShinobu/ShinobuTranslator/releases/download/models-v0.1.0/detector.onnx
...
INPUT_TAG="${1:?请指定版本tag，例如 v0.1.0 或 models-v0.1.0}"
TITLE="${2:-ONNX 模型 ${INPUT_TAG#models-}}"

# 模型 Release 统一使用 models-v* 前缀
...
if gh release view "$TAG" --repo "$REPO" &>/dev/null; then
  ...
  gh release create "$TAG" --repo "$REPO" --title "$TITLE" --notes "ONNX 模型文件，用于浏览器端漫画翻译推理" --prerelease
fi
...
echo "注意: 模型 Release 标记为 pre-release，不会显示为 Latest"
