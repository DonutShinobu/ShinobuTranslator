import type {
  ExtensionCookies,
  ExtensionPermissions,
  PermissionRequirement,
} from './contracts';
import {
  ExtensionContractError,
  ExtensionOperationError,
  sanitizedErrorDiagnostic,
} from './errors';
import {
  chromeCallback,
  idempotentCancel,
  isObject,
  requireFunction,
  requireNamespace,
  type ChromeCookie,
  type ChromeCookies,
  type ChromePermissionDetails,
  type ChromePermissions,
  type ChromeRuntime,
} from './chromeInternal';

function normalizedOrigin(
  requirement: PermissionRequirement,
): string | undefined {
  if (requirement.kind !== 'target-origin') return undefined;
  try {
    return `${new URL(requirement.origin).origin}/*`;
  } catch (error) {
    throw new ExtensionOperationError({
      capability: 'extension-permissions',
      operation: 'normalize-origin',
      code: 'serialization-failed',
      retryable: false,
      diagnostic: sanitizedErrorDiagnostic(error),
      cause: error,
    });
  }
}

function chromePermissionDetails(
  requirements: readonly PermissionRequirement[],
): {
  details: ChromePermissionDetails;
  browserRequirements: readonly PermissionRequirement[];
} {
  const browserRequirements = requirements.filter(
    (requirement) => requirement.kind !== 'authentication-data-use',
  );
  const permissions = browserRequirements.some(
    (requirement) => requirement.kind === 'cookie-access',
  )
    ? ['cookies']
    : undefined;
  const origins = browserRequirements.flatMap((requirement) => {
    const origin = normalizedOrigin(requirement);
    return origin ? [origin] : [];
  });
  return {
    details: {
      ...(permissions ? { permissions } : {}),
      ...(origins.length > 0 ? { origins } : {}),
    },
    browserRequirements,
  };
}

function changedRequirements(
  requirements: readonly PermissionRequirement[],
  details: ChromePermissionDetails,
): readonly PermissionRequirement[] {
  return requirements.filter((requirement) => {
    if (requirement.kind === 'authentication-data-use') return false;
    if (requirement.kind === 'cookie-access') {
      return details.permissions?.includes('cookies') ?? false;
    }
    const origin = normalizedOrigin(requirement);
    return origin ? details.origins?.includes(origin) ?? false : false;
  });
}

export function extensionPermissions(
  runtime: ChromeRuntime,
  rawPermissions: ChromePermissions | undefined,
): ExtensionPermissions {
  const permissions = requireNamespace(
    rawPermissions,
    'extension-permissions',
    'permissions',
  );
  requireFunction(permissions.contains, 'extension-permissions', 'check');
  requireFunction(permissions.request, 'extension-permissions', 'request');
  for (const [operation, event] of [
    ['onAdded', permissions.onAdded],
    ['onRemoved', permissions.onRemoved],
  ] as const) {
    if (!isObject(event)) {
      throw new ExtensionContractError({
        capability: 'extension-permissions',
        operation: 'onChanged',
        code: 'context-unavailable',
        retryable: false,
        diagnostic: {
          missing: operation,
        },
      });
    }
    requireFunction(event.addListener, 'extension-permissions', 'onChanged');
    requireFunction(event.removeListener, 'extension-permissions', 'cancel:onChanged');
  }

  const missingRequirements = async (
    requirements: readonly PermissionRequirement[],
  ): Promise<readonly PermissionRequirement[]> => {
    const browserRequirements = chromePermissionDetails(
      requirements,
    ).browserRequirements;
    const decisions = await Promise.all(browserRequirements.map(
      async (requirement) => {
        const { details } = chromePermissionDetails([requirement]);
        const granted = await chromeCallback<boolean>(
          runtime,
          'extension-permissions',
          'check',
          (complete) => permissions.contains(details, complete),
        );
        return granted ? undefined : requirement;
      },
    ));
    return decisions.filter(
      (requirement): requirement is PermissionRequirement => requirement !== undefined,
    );
  };

  const check = async (
    requirements: readonly PermissionRequirement[],
  ): ReturnType<ExtensionPermissions['check']> => {
    const mapped = chromePermissionDetails(requirements);
    if (mapped.browserRequirements.length === 0) {
      return { status: 'granted' };
    }
    const granted = await chromeCallback<boolean>(
      runtime,
      'extension-permissions',
      'check',
      (complete) => permissions.contains(mapped.details, complete),
    );
    if (granted) return { status: 'granted' };
    const missing = await missingRequirements(requirements);
    if (missing.length === 0) return { status: 'granted' };
    return {
      status: 'not-granted',
      missing,
    };
  };

  return {
    check,
    async request(requirements) {
      const mapped = chromePermissionDetails(requirements);
      if (mapped.browserRequirements.length === 0) {
        return { status: 'granted' };
      }
      const granted = await chromeCallback<boolean>(
        runtime,
        'extension-permissions',
        'request',
        (complete) => permissions.request(mapped.details, complete),
      );
      if (granted) return { status: 'granted' };
      const missing = await missingRequirements(requirements);
      return missing.length === 0
        ? { status: 'granted' }
        : {
            status: 'denied',
            missing,
          };
    },
    onChanged(requirements, listener) {
      const added = (details: ChromePermissionDetails): void => {
        const changed = changedRequirements(requirements, details);
        if (changed.length > 0) {
          listener({ status: 'granted', requirements: changed });
        }
      };
      const removed = (details: ChromePermissionDetails): void => {
        const changed = changedRequirements(requirements, details);
        if (changed.length > 0) {
          listener({ status: 'revoked', requirements: changed });
        }
      };
      permissions.onAdded.addListener(added);
      permissions.onRemoved.addListener(removed);
      return idempotentCancel(() => {
        permissions.onAdded.removeListener(added);
        permissions.onRemoved.removeListener(removed);
      });
    },
  };
}

export function extensionCookies(
  runtime: ChromeRuntime,
  rawCookies: ChromeCookies | undefined,
  permissions: ExtensionPermissions,
): ExtensionCookies {
  const cookies = requireNamespace(rawCookies, 'extension-cookies', 'cookies');
  requireFunction(cookies.getAll, 'extension-cookies', 'read');
  return {
    async read(query, requirements) {
      const permission = await permissions.check(requirements);
      if (permission.status === 'not-granted') {
        return {
          status: 'permission-required',
          missing: permission.missing,
        };
      }
      const values = await chromeCallback<ChromeCookie[]>(
        runtime,
        'extension-cookies',
        'read',
        (complete) => cookies.getAll(query, complete),
      );
      const normalized = values.map((cookie) => {
        for (const field of ['name', 'value', 'domain', 'path'] as const) {
          if (typeof cookie[field] !== 'string') {
            throw new ExtensionOperationError({
              capability: 'extension-cookies',
              operation: 'read',
              code: 'serialization-failed',
              retryable: false,
              diagnostic: {
                invalidField: field,
              },
            });
          }
        }
        return {
          name: cookie.name,
          value: cookie.value,
          domain: cookie.domain,
          path: cookie.path,
          secure: cookie.secure ?? false,
          httpOnly: cookie.httpOnly ?? false,
          ...(typeof cookie.expirationDate === 'number'
            ? { expirationDate: cookie.expirationDate }
            : {}),
        };
      });
      return {
        status: 'available',
        cookies: normalized,
      };
    },
  };
}
