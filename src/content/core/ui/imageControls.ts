...
export function renderUi(ui: UiElements, state: PhotoState | null): void {
  ...
  const prevIconKey =
    (buttonIcon.dataset.icon as IconKey | undefined) ?? 'translate';
  ...
  if (!isTransition) {
    replaceSvgIcon(buttonIcon, nextIconKey);
  ...
  buttonLabel.textContent = nextText;
  if (iconChange) replaceSvgIcon(buttonIcon, nextIconKey);
  button.style.width = '';
  ...
  if (iconChange) replaceSvgIcon(buttonIcon, prevIconKey);
