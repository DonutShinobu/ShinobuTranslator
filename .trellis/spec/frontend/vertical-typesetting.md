...
- 源列几何可靠时，初始视觉字号取 OCR/merge 区域字号与各源列局部字号中位数的较小值；源列 `height / glyphCount` 只用于纵向 advance，不得用最长原始列的字符数把整个区域压小。真实 mixed-token 布局和后续溢出收缩负责最终 fitting；源几何不可用时保留保守字符格估算。
